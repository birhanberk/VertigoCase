...
namespace UI.Wheel
{
    ...
    public class WheelSlotController
    {
        ...
        public void InitializeSlots(LevelSo levelSo, List<WheelItemView> slots, int levelIndex)
        {
            ...
            for (var i = 0; i < slots.Count; i++)
            {
                ...
                var itemSo = levelSo.Items[i];

                if (!itemSo)
                    itemSo = GetRandomItem(levelSo);

                if (itemSo == bombItem)
                    slot.UpdateItem(bombItem);
                else
                    slot.UpdateItem(itemSo, itemSo.GetAmount(levelIndex));
            }
        }
